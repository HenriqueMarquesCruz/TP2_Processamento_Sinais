...
%% FILTRO FIR
%% ------- Projeto FIR passa-baixas com janela de Kaiser -------

fprintf('\n=== Seção 1.3: Projeto FIR (janela de Kaiser) ===\n');
...
% Conversão para dB (aprox)
Ap_dB = 20*log10(1+Ap);
Ar_dB = 20*log10(1+Ar);
As = max(Ap_dB, Ar_dB);   % atenuação mínima

fprintf('Ripple em dB: As = %.6f dB\n', As);

% Largura de transição
df = fr - fp;             % Hz
dw = 2*pi*(df/fs);        % rad/s

% Ordem do filtro (Kaiser) — fórmula clássica
N = ceil( (As - 8) / (2.285 * dw) );
if N < 1, N = 1; end

% β da janela de Kaiser
if As > 50
    beta = 0.1102*(As - 8.7);
elseif As > 21
...
% Filtro FIR
fc = fp/(fs/2);     % normalização frequência de corte
h_fir = fir1(N, fc, kaiser(N+1, beta));

% --- Plota janela ---
fig2 = fixedFig('Janela de Kaiser', defaultFigPos);
figure(fig2);

subplot(2,1,1);
plot(kaiser(N+1,beta));
title(sprintf('Janela de Kaiser (N = %d, beta = %.4f)', N, beta));
xlabel('n'); ylabel('w[n]');
grid on;

% guarda para partes 1.4 e 1.5
save('fir_filter.mat','h_fir','N','beta');

%% FILTRO IIR
%% ------- Carregamento dos coeficientes do filtro IIR -------
...
fig3 = fixedFig('2. Resposta ao impulso truncada', defaultFigPos);
...
fig4 = fixedFig('3. Filtragem do sinal', defaultFigPos);
...
fig5 = fixedFig('4. BÔNUS: Implementação overlap-add', defaultFigPos);
