...
% Espectro de amplitude - CORRIGIDO: usar 2^nextpow2(N)
Nfft = 2^nextpow2(N);
X = fft(x, Nfft)/N;  % dividir ANTES do fftshift
Xs = fftshift(X);
...
fprintf('   Ordem do filtro N_fir = %d\n', N_fir);
...
% CORRIGIDO: calcular resposta do filtro para as frequências da STFT
% A STFT retorna apenas frequências positivas [0, fs/2]
for k = 1:num_frames
    if frames_with_noise(k)
        % Calcular resposta do filtro nas frequências da STFT
        H_fir_frame = freqz(h_fir, 1, F, fs);
        % Aplicar filtro multiplicando pela resposta em frequência
        S_filtered(:, k) = S(:, k) .* H_fir_frame;
    end
end
...
subplot(3,1,3);
plot(t, real(y_adaptive));  % CORRIGIDO: usar real() para garantir sinal real
xlabel('Tempo (s)'); ylabel('Amplitude');
...
if lower(choice) == 's'
    ...
    sound(real(y_adaptive), fs);  % CORRIGIDO: usar real()
    pause(N/fs + 0.5);
else
    fprintf('   Áudio não reproduzido.\n');
end
...
Y_adaptive = fftshift(fft(real(y_adaptive), Nfft))/N;  % CORRIGIDO: usar real()
...
subplot(2,2,4);
plot(t, abs(y_eqdif - real(y_adaptive)));  % CORRIGIDO: usar real()
