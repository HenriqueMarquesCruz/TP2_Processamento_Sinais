...
M = N_fir + 1;  % M = N_fir+1 (número de coeficientes)

fprintf('   Parâmetro Beta = %.4f\n', Beta);
fprintf('   Ordem do filtro N_fir = %d\n', N_fir);
fprintf('   Número de coeficientes M = %d\n\n', M);

% Gerar janela de Kaiser
w_kaiser = kaiser(N_fir, Beta);
...
n = 0:M;
alpha = M/2;  % atraso para tornar causal
...
subplot(2,1,1);
stem(0:N_fir-1, w_kaiser, 'filled');
xlabel('n');
ylabel('w[n]');
title(sprintf('Janela de Kaiser (N = %d, β = %.4f)', N_fir, Beta));
grid on;

subplot(2,1,2);
stem(0:N_fir-1, h_fir, 'filled');
xlabel('n');
ylabel('h[n]');
